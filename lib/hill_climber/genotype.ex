defmodule HillClimber.Genotype do
  alias HillClimber.Sensor
  alias HillClimber.Actuator
  alias HillClimber.Cortex
  alias HillClimber.Neuron
  ...
  def create_neuro_layers(cx_id, actuator_id, layer_index, tot_layers, input_id_ps, n_ids, [next_ld|lds], acc) do
    ...
    create_neuro_layers(cx_id, actuator_id, layer_index+1, tot_layers, next_input_id_ps, output_nids, lds, [layer_neurons|acc]);
  end
  ...
  def create_neuron(input_id_ps, id, cx_id, output_ids) do
    ...
    %Neuron{id: id, cx_id: cx_id, af: :tanh, input_id_ps: proper_input_id_ps, output_ids: output_ids}
  end
  ...
  @doc """
  The `save_genotype/2` function expects that the Genotype is a list composed of
  the neuron, sensor, actuator, cortex, and exoself elements. The function
  creates a new ets table, writes all the element representing tuples from the
  Genotype list to the ets table, and then writes the ets table to file.
  """
  def save_genotype(file_name, genotype) do
    table_id = :ets.new(file_name, [:public, :set, {:keypos, 1}])
    for element <- genotype, do: :ets.insert(table_id, {element.name, element})
    :ets.tab2file(table_id, file_name)
  end

  @doc """
  The `save_to_file/2` function saves the ets table by the name Genotype to the
  file by the name file_name.
  """
  ...
  @doc """
  The load_from_file/1 loads an ets representing file by the name file_name, returning the ets table id to the caller.
  """
  ...
  def read(table_id, key) do
    [{^key, struct}] = :ets.lookup(table_id, key)
    struct
  end
  ...
  """
  def write(table_id, struct) do
    :ets.insert(table_id, {struct.name, struct})
  end

  @doc """
  The function print/1 reads a stored Genotype from the file file_name, and then prints to console all the elements making up the NN’s genotype.
  """
  def print(file_name) do
    genotype = load_from_file(file_name)
    cortex = read(genotype, :cortex)
    Logger.info "cortex: #{inspect(cortex)}"
    for id <- cortex.sensor_ids, do: IO.inspect read(genotype, id)
    for id <- cortex.n_ids, do: IO.inspect read(genotype, id)
    for id <- cortex.actuator_ids, do: IO.inspect read(genotype, id)
  end
end
